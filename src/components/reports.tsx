              ...
              <Card key={supplier.name} className="flex flex-col">
                ...
                <div className="p-4 pt-0 mt-2">
                  <div className="bg-muted rounded-lg p-3 text-center">
                    ...
                    <p className="text-2xl font-bold text-primary">{supplier.totalCost.toFixed(2)} rupees</p>
                  </div>
                </div>
              </Card>
            ))}
          ...
          <CardHeader>
            ...
            <div className="flex gap-6 items-center mt-2">
              <div className="flex items-center gap-2">
                <Checkbox id="expenses-cb" checked={showExpenses} onCheckedChange={v => setShowExpenses(!!v)} />
                <Label htmlFor="expenses-cb">Expenses</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox id="milk-cb" checked={showMilk} onCheckedChange={v => setShowMilk(!!v)} />
                <Label htmlFor="milk-cb">Milk</Label>
              </div>
            </div>
            <CardDescription>
              {showExpenses && showMilk
                ? 'Showing both expenses and milk cost for the selected range.'
                : showExpenses
                  ? 'Showing expenses for the selected range.'
                  : showMilk
                    ? 'Showing milk cost for the selected range.'
                    : 'Select at least one to display.'}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={dailyTotals} margin={{ top: 16, right: 16, left: 8, bottom: 32 }}>
                ...
                {showExpenses && <Bar dataKey="Expenses" fill="hsl(var(--chart-1))" radius={0} />}
                {showMilk && <Bar dataKey="Milk" fill="hsl(var(--chart-2))" radius={0} />}
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <CardContent className="pl-2">
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={cumulativeSpending}>
              ...
              <YAxis width={60} tickFormatter={(value) => value} />
              <Tooltip cursor={{ fill: 'hsl(var(--muted))' }} contentStyle={{ backgroundColor: 'hsl(var(--background))' }} />
              <Area type="monotone" dataKey="Spending" stroke="hsl(var(--primary))" fill="hsl(var(--primary) / 0.2)" strokeLinejoin="miter" strokeLinecap="butt" />
            </AreaChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
